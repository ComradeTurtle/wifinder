-- ESPWIGLE BLE ATT post-dissector for Wireshark.
--
...
if rawget(_G, "__espwigle_dissector_loaded") then
  return
end
_G.__espwigle_dissector_loaded = true

local espwigle = Proto("espwigle", "ESPWIGLE")
...
local pf_att_opcode = ProtoField.uint8("espwigle.att_opcode", "ATT Opcode", base.HEX)
local pf_att_handle = ProtoField.uint16("espwigle.att_handle", "ATT Handle", base.HEX)

local pf_magic = ProtoField.string("espwigle.magic", "Magic")
local pf_version = ProtoField.uint8("espwigle.version", "Version", base.DEC)
local pf_type = ProtoField.uint8("espwigle.type", "Type", base.HEX, MSG_NAMES)
local pf_seq = ProtoField.uint16("espwigle.seq", "Sequence", base.DEC)
local pf_len = ProtoField.uint16("espwigle.len", "Payload Length", base.DEC)
local pf_device_ms = ProtoField.uint32("espwigle.device_ms", "Device Milliseconds", base.DEC)
local pf_payload_note = ProtoField.string("espwigle.payload_note", "Payload")
local pf_parse_error = ProtoField.string("espwigle.parse_error", "Parse Error")

local pf_status_scanning = ProtoField.uint8("espwigle.status.scanning", "Status Scanning", base.DEC,
  { [0] = "false", [1] = "true" })
local pf_status_ble_encrypted = ProtoField.uint8("espwigle.status.ble_encrypted", "Status BLE Encrypted", base.DEC,
  { [0] = "false", [1] = "true" })
local pf_status_channel = ProtoField.uint8("espwigle.status.channel", "Status Current Channel", base.DEC)
local pf_status_hop_ms = ProtoField.uint16("espwigle.status.hop_ms", "Status Hop Interval (ms)", base.DEC)
local pf_status_channel_mask = ProtoField.uint16("espwigle.status.channel_mask", "Status Channel Mask", base.HEX)
local pf_status_unique = ProtoField.uint16("espwigle.status.unique_bssids", "Status Unique BSSIDs", base.DEC)
local pf_status_pps = ProtoField.uint16("espwigle.status.packets_per_sec", "Status Packets/s", base.DEC)
local pf_status_drops = ProtoField.uint16("espwigle.status.dropped_notifies", "Status Dropped Notifies", base.DEC)
local pf_status_boot = ProtoField.uint8("espwigle.status.boot_mode", "Status Boot Mode", base.DEC,
  { [0] = "manual", [1] = "auto" })
local pf_status_gps_valid = ProtoField.uint8("espwigle.status.gps_valid", "Status GPS Valid", base.DEC,
  { [0] = "false", [1] = "true" })
local pf_status_gps_age_s = ProtoField.uint16("espwigle.status.gps_age_s", "Status GPS Age (s)", base.DEC)
local pf_status_gps_acc_dm = ProtoField.uint16("espwigle.status.gps_accuracy_dm", "Status GPS Accuracy (dm)",
  base.DEC)
local pf_status_node_link_up = ProtoField.uint8("espwigle.status.node_link_up", "Status Node Link Up", base.DEC,
  { [0] = "false", [1] = "true" })
local pf_status_node_last_seen_s = ProtoField.uint16("espwigle.status.node_last_seen_s", "Status Node Last Seen (s)",
  base.DEC)
local pf_status_node_pps = ProtoField.uint16("espwigle.status.node_packets_per_sec", "Status Node Packets/s", base.DEC)
local pf_status_node_forwarded = ProtoField.uint16("espwigle.status.node_forwarded_sightings",
  "Status Node Forwarded Sightings", base.DEC)
local pf_status_node_channel = ProtoField.uint8("espwigle.status.node_channel", "Status Node Channel", base.DEC)
local pf_status_node_channel_mask = ProtoField.uint16("espwigle.status.node_channel_mask", "Status Node Channel Mask",
  base.HEX)
local pf_status_session_open = ProtoField.uint8("espwigle.status.session_open", "Status Session Open", base.DEC,
  { [0] = "false", [1] = "true" })
local pf_status_session_id = ProtoField.string("espwigle.status.session_id", "Status Session ID")
local pf_status_queued_records = ProtoField.uint32("espwigle.status.queued_records", "Status Queued Records", base.DEC)
local pf_status_queued_bytes = ProtoField.uint32("espwigle.status.queued_bytes", "Status Queued Bytes", base.DEC)
local pf_status_replay_active = ProtoField.uint8("espwigle.status.replay_active", "Status Replay Active", base.DEC,
  { [0] = "false", [1] = "true" })
local pf_status_replay_cursor = ProtoField.uint32("espwigle.status.replay_cursor", "Status Replay Cursor Seq", base.DEC)
local pf_status_queue_full = ProtoField.uint8("espwigle.status.queue_full", "Status Queue Full", base.DEC,
  { [0] = "false", [1] = "true" })
local pf_status_dropped_flash_full = ProtoField.uint32("espwigle.status.dropped_flash_full",
  "Status Dropped (Flash Full)", base.DEC)
local pf_status_node_count = ProtoField.uint8("espwigle.status.node_count", "Status Node Count", base.DEC)

local pf_gps_valid = ProtoField.uint8("espwigle.gps.valid", "GPS Valid", base.DEC,
  { [0] = "false", [1] = "true" })
local pf_gps_source = ProtoField.uint8("espwigle.gps.source", "GPS Source", base.DEC,
  { [0] = "none", [1] = "uart", [2] = "phone" })
local pf_gps_lat = ProtoField.int32("espwigle.gps.lat_e7", "GPS Latitude (e7)", base.DEC)
local pf_gps_lon = ProtoField.int32("espwigle.gps.lon_e7", "GPS Longitude (e7)", base.DEC)
local pf_gps_alt = ProtoField.int32("espwigle.gps.alt_mm", "GPS Altitude (mm)", base.DEC)
local pf_gps_speed = ProtoField.uint32("espwigle.gps.speed_mmps", "GPS Speed (mm/s)", base.DEC)
local pf_gps_bearing = ProtoField.uint32("espwigle.gps.bearing_mdeg", "GPS Bearing (mdeg)", base.DEC)
local pf_gps_time = ProtoField.uint32("espwigle.gps.unix_time_s", "GPS Unix Time (s)", base.DEC)
local pf_gps_accuracy = ProtoField.uint16("espwigle.gps.accuracy_cm", "GPS Accuracy (cm)", base.DEC)
local pf_gps_sat_count = ProtoField.uint8("espwigle.gps.sat_count", "GPS Satellites", base.DEC)
local pf_gps_hdop_centi = ProtoField.uint16("espwigle.gps.hdop_centi", "GPS HDOP (x100)", base.DEC)
local pf_gps_pdop_centi = ProtoField.uint16("espwigle.gps.pdop_centi", "GPS PDOP (x100)", base.DEC)

local pf_sighting_bssid = ProtoField.string("espwigle.sighting.bssid", "Sighting BSSID")
local pf_sighting_channel = ProtoField.uint8("espwigle.sighting.channel", "Sighting Channel", base.DEC)
local pf_sighting_rssi = ProtoField.int8("espwigle.sighting.rssi", "Sighting RSSI", base.DEC)
local pf_sighting_auth = ProtoField.uint8("espwigle.sighting.auth", "Sighting Auth", base.DEC, AUTH_NAMES)
local pf_sighting_proto = ProtoField.uint8("espwigle.sighting.proto_flags", "Sighting Protocol Flags", base.HEX)
local pf_sighting_akm = ProtoField.uint8("espwigle.sighting.akm_flags", "Sighting AKM Flags", base.HEX)
local pf_sighting_cipher = ProtoField.uint8("espwigle.sighting.cipher_flags", "Sighting Cipher Flags", base.HEX)
local pf_sighting_ssid_len = ProtoField.uint8("espwigle.sighting.ssid_len", "Sighting SSID Length", base.DEC)
local pf_sighting_ssid = ProtoField.string("espwigle.sighting.ssid", "Sighting SSID")
local pf_sighting_flags = ProtoField.uint8("espwigle.sighting.flags", "Sighting Flags", base.HEX)
local pf_sighting_session_id = ProtoField.string("espwigle.sighting.session_id", "Sighting Session ID")
local pf_sighting_record_seq = ProtoField.uint32("espwigle.sighting.record_seq", "Sighting Record Seq", base.DEC)
local pf_sighting_node_id = ProtoField.uint8("espwigle.sighting.node_id", "Sighting Node ID", base.DEC)
local pf_sighting_source_flags = ProtoField.uint8("espwigle.sighting.source_flags", "Sighting Source Flags", base.HEX)
local pf_sighting_gps_valid = ProtoField.uint8("espwigle.sighting.gps_valid", "Sighting GPS Valid", base.DEC,
  { [0] = "false", [1] = "true" })
local pf_sighting_gps_source = ProtoField.uint8("espwigle.sighting.gps_source", "Sighting GPS Source", base.DEC,
  { [0] = "none", [1] = "uart", [2] = "phone" })
local pf_sighting_gps_lat = ProtoField.int32("espwigle.sighting.gps_lat_e7", "Sighting GPS Latitude (e7)", base.DEC)
local pf_sighting_gps_lon = ProtoField.int32("espwigle.sighting.gps_lon_e7", "Sighting GPS Longitude (e7)", base.DEC)
local pf_sighting_gps_alt = ProtoField.int32("espwigle.sighting.gps_alt_mm", "Sighting GPS Altitude (mm)", base.DEC)
local pf_sighting_gps_time = ProtoField.uint32("espwigle.sighting.gps_unix_time_s", "Sighting GPS Unix Time (s)",
  base.DEC)
local pf_sighting_gps_accuracy = ProtoField.uint16("espwigle.sighting.gps_accuracy_cm",
  "Sighting GPS Accuracy (cm)", base.DEC)

local pf_ack_cmd = ProtoField.uint8("espwigle.ack.command", "ACK Command", base.HEX, CMD_NAMES)
local pf_error_cmd = ProtoField.uint8("espwigle.error.command", "Error Command", base.HEX, CMD_NAMES)
local pf_error_code = ProtoField.uint8("espwigle.error.code", "Error Code", base.DEC)

local pf_cfg_hop = ProtoField.uint16("espwigle.config.hop_ms", "Config Hop Interval (ms)", base.DEC)
local pf_cfg_mask = ProtoField.uint16("espwigle.config.channel_mask", "Config Channel Mask", base.HEX)
local pf_cfg_boot = ProtoField.uint8("espwigle.config.boot_mode", "Config Boot Mode", base.DEC,
  { [0] = "manual", [1] = "auto" })

local pf_cmd_id = ProtoField.uint8("espwigle.command.id", "Command ID", base.HEX, CMD_NAMES)
local pf_cmd_hop = ProtoField.uint16("espwigle.command.hop_ms", "Command Hop Interval (ms)", base.DEC)
local pf_cmd_mask = ProtoField.uint16("espwigle.command.channel_mask", "Command Channel Mask", base.HEX)
local pf_cmd_boot = ProtoField.uint8("espwigle.command.boot_mode", "Command Boot Mode", base.DEC,
  { [0] = "manual", [1] = "auto" })
local pf_cmd_gps_flags = ProtoField.uint8("espwigle.command.gps_flags", "Command GPS Flags", base.HEX)
local pf_cmd_gps_lat = ProtoField.int32("espwigle.command.gps_lat_e7", "Command GPS Latitude (e7)", base.DEC)
local pf_cmd_gps_lon = ProtoField.int32("espwigle.command.gps_lon_e7", "Command GPS Longitude (e7)", base.DEC)
local pf_cmd_gps_alt = ProtoField.int32("espwigle.command.gps_alt_mm", "Command GPS Altitude (mm)", base.DEC)
local pf_cmd_gps_speed = ProtoField.uint32("espwigle.command.gps_speed_mmps", "Command GPS Speed (mm/s)", base.DEC)
local pf_cmd_gps_bearing = ProtoField.uint32("espwigle.command.gps_bearing_mdeg", "Command GPS Bearing (mdeg)",
  base.DEC)
local pf_cmd_gps_time = ProtoField.uint32("espwigle.command.gps_unix_time_s", "Command GPS Unix Time (s)", base.DEC)
local pf_cmd_gps_accuracy = ProtoField.uint16("espwigle.command.gps_accuracy_cm", "Command GPS Accuracy (cm)",
  base.DEC)
local pf_cmd_replay_sid = ProtoField.string("espwigle.command.replay_session_id", "Command Replay Session ID")
local pf_cmd_replay_seq = ProtoField.uint32("espwigle.command.replay_highest_seq", "Command Replay Highest Seq",
  base.DEC)
local pf_cmd_replay_enable = ProtoField.uint8("espwigle.command.replay_enable", "Command Replay Enable",
...
function espwigle.dissector(_, pinfo, tree)
  ...
  local root = tree:add(espwigle, value_fi.range, "ESPWIGLE Frame")
...
register_postdissector(espwigle)
